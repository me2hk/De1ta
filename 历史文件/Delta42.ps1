...
namespace WindowControl 
{
    ...
    public class InputControl
    {
        ...
        private const int TIME_WINDOW = 2000;        // 点击检测时间窗口(毫秒)
        private const int STUTTER_TIMEOUT = 2000;    // 碎步后等待Shift松开的超时时间(毫秒)
        ...
        public static void Start() 
        {
            while(true) 
            {
                    ...
                    else
                    {
                        ...
                        if (wCurrentPressed && !isWPressed)
                        {
                                ...
                                (DateTime.Now - firstPressTime).TotalMilliseconds > TIME_WINDOW)
                            {
                                ...
                                // Console.WriteLine(string.Format("检测到第 {0} 次W键按下", wPressCount));
                            }
                            else
                            {
                                wPressCount++;
                                    ...
                                    (DateTime.Now - firstPressTime).TotalMilliseconds <= TIME_WINDOW)
                                {
                                    ...
                                    stutterDetectedTime = DateTime.Now;  // 记录碎步检测时间
                                    // Console.WriteLine("检测到碎步行为，请在2秒内松开Shift键");
                                }
                            }
                        }
                ...
                else
                {
                    ...
                    if (isStutterDetected)
                    {
                        // Console.WriteLine("松开Shift键，触发Alt键操作!");
                        // 模拟Alt键按下和松开
                    ...
                    {
                        // Console.WriteLine("Shift键松开，重置所有状态");
                        wPressCount = 0;
                        firstPressTime = DateTime.MinValue;
                        isStutterDetected = false;
                        stutterDetectedTime = DateTime.MinValue;
