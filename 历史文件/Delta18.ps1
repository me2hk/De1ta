...
namespace CombatControl 
{
    ...
    public class FireControl 
    {
        ...
        private const int VK_LSHIFT = 0xA0;          // 左Shift键 (碎步)
        private const int VK_RSHIFT = 0xA1;          // 右Shift键 (瞄准)
        private const int VK_LMENU = 0xA4;           // 左Alt键   (静步)
        private const int VK_RMENU = 0xA5;           // 右Alt键   (屏息)
        ...
        //压枪控制常量
        private static WeaponConfig[] allWeapons;          // 所有可用武器配置
        private static WeaponConfig currentWeapon;         // 当前选择的武器配置
        ...
        private static bool lastAddKeyState = false;      // 跟踪[+]键状态
        ...
        // 加载XML配置
        public static WeaponConfig[] LoadConfig(string filePath)
        {
            try
            {
                ...
                WeaponConfig[] weapons = new WeaponConfig[weaponNodes.Count];
                
                // 遍历所有的 <Weapon> 节点
                for (int i = 0; i < weaponNodes.Count; i++)
                {
                    // 创建一个新的 WeaponConfig 对象
        ...
        public static void Start() 
        {
            // 预加载xml武器预设
            if (!configLoaded) {
                allWeapons = LoadConfig("WeaponConfig.xml");
                configLoaded = true;
            ...
            while(true) 
            {
                ...
                // 自动压枪开关检测(带二次确认防抖动)
                bool currentAddKey = (GetAsyncKeyState(VK_NUMPAD_ADD) & KEY_PRESSED_FLAG) != 0;
                if(currentAddKey && !lastAddKeyState) 
                {
                    ...
                    if((GetAsyncKeyState(VK_NUMPAD_ADD) & KEY_PRESSED_FLAG) != 0)
                    {
                        if (!isRecoilEnabled) 
                        {
                            ...
                            for (int i = 0; i < allWeapons.Length; i++) {
                                Console.WriteLine("\x1B[32m    {0}.{1,-5}\x1B[0m", 
                                    i + 1, 
                                    allWeapons[i].Name, 
                ...
                lastAddKeyState = currentAddKey; // 重置状态跟踪
                ...
                if(currentNumpadSub && !lastNumpadSubState) 
                {
                    ...
                    if((GetAsyncKeyState(VK_NUMPAD_SUB) & KEY_PRESSED_FLAG) != 0)
                    {
                        ...
                        // 瞄准屏息状态提示(使用ANSI颜色代码)
                        Console.WriteLine("[-] 瞄准屏息: " + (isBreathEnabled ? "\x1B[32m开启 (右Shift/右Alt键)\x1B[0m" : "\x1B[31m关闭\x1B[0m"));
                    }
                }
                ...
                if(currentMultiply && !lastMultiplyState) 
                {
                    ...
                    if((GetAsyncKeyState(VK_NUMPAD_MULTIPLY) & KEY_PRESSED_FLAG) != 0)
                    {
                        ...
                        // 自动碎步状态提示(使用ANSI颜色代码)
                        Console.WriteLine("[*] 自动碎步: " + (isStutterStepEnabled ? "\x1B[32m开启 (左Shift/左Alt键)\x1B[0m" : "\x1B[31m关闭\x1B[0m"));
                    }
                }
                ...
                if (triggerActive && currentWeapon != null && isRecoilEnabled)
                {
                    if (currentStage == 0)
                    {
                        ...
                        .Select(s => s.Duration + rand.Value.Next(-100, 101))
                        .ToArray();
                    }
                    ...
                    if (currentStage <= currentWeapon.Stages.Length)
                    {
                        ...
                        if (currentStage <= currentWeapon.Stages.Length)
                        {
                            ...
                            // 生成垂直±随机值
                            int dy = stage.VerticalOffset + rand.Value.Next(-1, 2);

                            // 生成水平±随机值
                            int dx = stage.HorizontalJitter + rand.Value.Next(-1, 2);
                            ...
                            // 生成一个随机的延迟时间
                            int delay = (int)(rand.Value.NextGaussian() * 5 + 30);
                            // 确保延迟时间在 20 到 50 毫秒之间
                            delay = Math.Max(20, Math.Min(50, delay));
                            // 线程休眠指定的延迟时间
                            Thread.Sleep(delay);
                        }
                    }
                ...
                if(isBreathEnabled) 
                {
                    ...
                    {
                        keybd_event((byte)VK_RSHIFT, 0x36, 0x0001, 0);  // 模拟右Shift按下 (瞄准)
                        Thread.Sleep(rand.Value.Next(5, 16)); // 随机5-15ms
                        keybd_event((byte)VK_RMENU, 0xB8, 0x0001, 0);   // 模拟右Alt按下 (屏息)
                    } 
                    else if(!qOrEState && lastQOrEState) 
                    {
                        keybd_event((byte)VK_RMENU, 0xB8, 0x0002, 0);     // 模拟右Alt释放 (屏息)
                        Thread.Sleep(rand.Value.Next(5, 16)); // 随机5-15ms
                        keybd_event((byte)VK_RSHIFT, 0x36, 0x0002, 0);    // 模拟右Shift释放 (瞄准)
                    }
                ...
                if(isStutterStepEnabled) 
                {
                    ...
                    {
                        // 模拟左Alt键按下+释放
                        keybd_event((byte)VK_LMENU, 0x38, 0x0000, 0); // 模拟左Alt按下
                        Thread.Sleep(rand.Value.Next(5, 16)); // 随机5-15ms
                        keybd_event((byte)VK_LMENU, 0x38, 0x0002, 0);  //模拟左Alt释放

                        keybd_event((byte)VK_LSHIFT, 0x2A, 0x0002, 0);  // 强制释放左Shift键
                    ...
                    if(isStutterActive)
                    {
                        ...
                        int randomDelay = rand.Value.Next(70, 91);
                                ...
                                {
                                   // 模拟按键周期(保持按下时间动态调整)
                                   keybd_event((byte)VK_LSHIFT, 0x2A, 0x0000, 0); // 模拟左Shift按下
                                   Thread.Sleep(randomDelay);  // 保持时间与间隔同步随机化
                                   keybd_event((byte)VK_LSHIFT, 0x2A, 0x0002, 0); // 模拟左Shift释放
...
# 窗口初始化后立即加载武器配置
try {
    $global:allWeapons = [CombatControl.FireControl]::LoadConfig("$PSScriptRoot\WeaponConfig.xml")
...
Write-Host "[-] 瞄准屏息: $([char]27)[32m开启 (右Shift/右Alt键)$([char]27)[0m"
Write-Host "[*] 自动碎步: $([char]27)[32m开启 (左Shift/左Alt键)$([char]27)[0m"
