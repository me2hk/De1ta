...
namespace CombatControl 
{
    ...
    public class FireControl 
    {
        ...
        private const byte VK_LMENU = 0xA4;     //Alt键
        ...
        private const int VK_NUMPAD_DIV = 0x6F;      // [/] 键
        ...
        public static bool isAutoLootEnabled = true;  // 自动拾取开关值
        ...
        private static bool lastNumpadDivState = false; // 跟踪[/]键状态
        private static DateTime lastStutterTime = DateTime.MinValue; // 碎步时间戳
        private static DateTime rightButtonDownTime = DateTime.MinValue; // 拾取时间戳
        ...
        public static void Start() 
        {
            while(true) 
            {
                ...
                // 自动拾取开关检测
                bool currentNumpadDiv = (GetAsyncKeyState(VK_NUMPAD_DIV) & KEY_PRESSED_FLAG) != 0;
                if(currentNumpadDiv && !lastNumpadDivState) 
                {
                    Thread.Sleep(10); // 10ms延时消抖
                    isAutoLootEnabled = !isAutoLootEnabled; // 切换自动拾取开关
    
                        // 自动拾取状态提示(使用ANSI颜色)
                        Console.WriteLine("[/] 自动拾取: " + (isAutoLootEnabled ? "\x1B[32m映射 (鼠标左键)\x1B[0m" : "\x1B[31m关闭\x1B[0m"));
                    }
                    lastNumpadDivState = currentNumpadDiv; // 重置状态跟踪
                ...
                if(isRecoilEnabled) 
                {
                    ...
                    if(qOrEPressed && mouseState) 
                    {
                        ...
                        // 高斯分布随机延迟模型：μ=30ms, σ=20ms
                        double gaussian = rand.Value.NextGaussian();
                        int delay = (int)(gaussian * 20 + 30);  // μ=30ms, σ=20ms 的正态分布

                        //添加噪声
                        int noise = rand.Value.Next(-2, 3); // ±2ms随机干扰
                        delay += noise;   // 应用噪声

                            // 随机延迟范围限制30-50ms
                        delay = Math.Max(28, Math.Min(delay, 52));  // 允许临时超出2ms
                        delay = Math.Max(30, Math.Min(delay, 50));  // 最终限制到目标范围
                ...
                if(isBreathEnabled) 
                {
                    ...
                    if(qOrEState && !lastQOrEState) 
                    {
                        ...
                        Thread.Sleep(rand.Value.Next(1, 6)); // 随机1-5ms
                ...
                if(isStutterStepEnabled) 
                {
                    ...
                    else if((!wPressed || !capsLockPressed) && isStutterActive)
                    {
                        ...
                        Thread.Sleep(rand.Value.Next(5, 11)); // 随机5-10ms
                        keybd_event(VK_LMENU, 0, KEY_UP_FLAG, 0);  //模拟Alt释放

                        // 强制Shift释放
                        keybd_event((byte)VK_SHIFT, 0, KEY_UP_FLAG, 0);  // 强制释放Shift键
                ...
                // 自动拾取功能实现(当鼠标右键按下0.6秒后触发)
                if (isAutoLootEnabled) 
                {
                    bool rButtonState = (GetAsyncKeyState(VK_RBUTTON) & KEY_PRESSED_FLAG) != 0;
    
                    // 记录右键按下时间
                    if (rButtonState && rightButtonDownTime == DateTime.MinValue) 
                    {
                        rightButtonDownTime = DateTime.Now;
                    }
    
                    // 检测长按超过0.6秒
                    if (rButtonState && (DateTime.Now - rightButtonDownTime).TotalSeconds >= 0.6) 
                    {
                        // 模拟按下左键(带随机偏移)
                        mouse_event(MOUSEEVENTF_LEFTDOWN, rand.Value.Next(-1, 2), rand.Value.Next(-1, 2), 0, 0);

                        // 按压保持时间(55-75ms)
                        Thread.Sleep(rand.Value.Next(55, 76));
        
                        // 模拟释放左键
                        mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);

                        // 点击间隔(25-30ms)
                        Thread.Sleep(rand.Value.Next(25, 31));
                    }
...
Write-Host "[*] 自动拾取: $([char]27)[32m映射 (鼠标左键)$([char]27)[0m"
...
try {
    ...
    if($_.Exception.NativeErrorCode -eq 2) {
        ...
        Write-Host "[?] 游戏进程: $([char]27)[31m未知错误$([char]27)[0m"
        Write-Host "" 
}
