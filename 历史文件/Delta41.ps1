...
namespace WindowControl 
{
    ...
    public class InputControl
    {
        ...
        public struct INPUT
        {
            public uint type;          // 输入类型：1=键盘，2=鼠标，3=硬件
            public InputUnion U;       // 联合结构体，存储具体输入信息
        }
        ...
        {
            [FieldOffset(0)]
            public MOUSEINPUT mi;      // 鼠标输入信息
            [FieldOffset(0)]
            public KEYBDINPUT ki;      // 键盘输入信息
            [FieldOffset(0)]
            public HARDWAREINPUT hi;   // 硬件输入信息
        }
        ...
        {
            public uint uMsg;          // 硬件消息
            public ushort wParamL;     // 参数低16位
            public ushort wParamH;     // 参数高16位
        }

        // 虚拟键码常量
        private const ushort VK_W = 0x57;                // W键
        private const ushort VK_CAPITAL = 0x14;          // Caps Lock键
        private const ushort VK_LSHIFT = 0xA0;           // Shift键 (碎步)
        private const ushort VK_LMENU = 0xA4;            // Alt键   (静步)
        ...
        // 按键状态跟踪
        private static bool isStutterActive = false;     // 跟踪碎步状态
        ...
        private static void PressKey(ushort virtualKey)
        {
            ...
            input[0].type = 1; // 类型：键盘输入
            input[0].U.ki.wVk = virtualKey;
            input[0].U.ki.wScan = 0;
            input[0].U.ki.dwFlags = KEYEVENTF_KEYDOWN;
            input[0].U.ki.time = 0;
        ...
        // 模拟按键释放
        private static void ReleaseKey(ushort virtualKey)
        {
            INPUT[] input = new INPUT[1];
            input[0].type = 1; // 类型：键盘输入
            input[0].U.ki.wVk = virtualKey;
        ...
        public static void Start() 
        {
            ...
            {
                // 单个按键状态声明
                bool wPressed = (GetAsyncKeyState(VK_W) & KEY_PRESSED_FLAG) != 0;                // W键
                bool capsLockPressed = (GetAsyncKeyState(VK_CAPITAL) & KEY_PRESSED_FLAG) != 0;   // Caps Lock键

                // 组合按键状态声明
                bool comboPressed = wPressed && capsLockPressed;                                 // W+Caps Lock键


                // 自动碎步功能实现(当W+Caps Lock键按下时触发)
                ...
                {
                    // 模拟Alt键按下与释放（静步）
                    PressKey(VK_LMENU);                                 // 模拟Alt按下
                    Thread.Sleep(rand.Value.Next(10, 61));              // 随机1-60ms
                    ReleaseKey(VK_LMENU);                               // 模拟Alt释放
                    Thread.Sleep(rand.Value.Next(10, 61));              // 随机1-60ms
                    ReleaseKey(VK_LSHIFT);                              // 强制释放Shift键

                    isStutterActive = false;                            // 重置状态
                    lastStutterTime = DateTime.MinValue;                // 重置时间戳
                }

                // 执行碎步操作(每60ms-160ms发送一次Shift)
                if(isStutterActive)
                {
                       // 高斯分布随机延迟
                       ...
                       // 5%概率触发突发延迟
                       if (rand.Value.NextDouble() < 0.05)              // 5%概率
                       {
                           delay = rand.Value.Next(100, 161);           // 100-160ms随机延迟
                       }
                       else
                       {
                           delay = (int)(gaussian * 7 + 80);            // 高斯分布参数：μ=7ms(中心点), σ=80ms(标准差) 
                           ...
                           // 随机延迟范围限制
                           delay = Math.Max(55, Math.Min(delay, 105));  // 允许临时超出
                           delay = Math.Max(60, Math.Min(delay, 100));  // 最终限制到目标范围
                       }

                       Thread.Sleep(delay);

                       if ((DateTime.Now - lastStutterTime).TotalMilliseconds >= delay)
                       {
                           // 模拟按键周期
                           PressKey(VK_LSHIFT);                         // 模拟Shift按下
                           Thread.Sleep(delay);                         // 保持时间与间隔同步
                           ReleaseKey(VK_LSHIFT);                       // 模拟Shift释放
