            ...
            while(true) 
            {
                ...
                if(currentAddKey && !lastAddKeyState) 
                {
                    ...
                    if((GetAsyncKeyState(VK_NUMPAD_ADD) & KEY_PRESSED_FLAG) != 0)
                    {
                        ...
                        ClearInputBuffer();
                         Console.WriteLine("");
                        Console.WriteLine("[+] 枪支预设");
                        for (int i = 0; i < allWeapons.Length; i++)
                        {
                            Console.WriteLine("    {0}. {1} - 阶段数: {2}", 
                                i + 1, 
                                allWeapons[i].Name, 
                        ...
                        while (true)
                        {
                            Console.Write("[+] 选择枪支: ", allWeapons.Length);
                            int choice;
                            if (int.TryParse(Console.ReadLine(), out choice) && choice >= 1 && choice <= allWeapons.Length)
                            {
                                currentWeapon = allWeapons[choice - 1];
                                Console.WriteLine("已选择武器: {0}", currentWeapon.Name);
                                break;
                            }
                ...
                if (triggerActive && currentWeapon != null)
                {
                    if (currentStage == 0)
                    {
...
Write-Host ""
Write-Host ""
Write-Host "[+] 自动压枪: $([char]27)[32m选择 (PTR-32)$([char]27)[0m"
