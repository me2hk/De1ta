...
namespace CombatControl 
{
    ...
    public class FireControl 
    {
        ...
        private const int VK_HOME = 0x24;      // Home键
        ...
        public static void Start() 
        {
            while(true) 
            {
                ...
                if(isBreathEnabled) 
                {
                    ...
                    {
                        keybd_event((byte)VK_HOME, 0, KEY_DOWN_FLAG, 0); // 模拟Home按下
                        Thread.Sleep(3);
                    ...
                    else if(!qOrEState && lastQOrEState) 
                    {
                        ...
                        Thread.Sleep(3);
                        keybd_event((byte)VK_HOME, 0, KEY_DOWN_FLAG, 0); // 模拟Home释放
                    }
...
Write-Host "[-] 自动屏息 (固定映射:Insert/Home键)"
Write-Host "[*] 自动碎步 (固定映射:Shift键)"

# 进程检测与交互逻辑
do {
    $processName = "delta_force_launcher"
    $launcherPath = "D:\Delta Force\launcher\delta_force_launcher.exe"  # 绝对路径定义
    
    $process = Get-Process -Name $processName -ErrorAction SilentlyContinue
    if ($process) {
        Write-Host "[?] 游戏进程 " -NoNewline
        Write-Host "(已启动)"
        break
    } else {
        # 带路径的启动命令
