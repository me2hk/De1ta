...
namespace De1ta
{
    ...
    public static class ProcessMonitor
    {
        ...
        {
            string targetProcess = "UnrealCEFSubProcess";                      // 监控的进程名
            bool hotKeyPressed = false;                                        // 热键按下状态标志

            // 持续监控循环
            while (isMonitoring)
            {
                // 检查热键Ctrl+Home是否按下
                bool ctrlPressed = (GetAsyncKeyState(VK_CONTROL) & KEY_PRESSED) != 0;
                bool homePressed = (GetAsyncKeyState(VK_HOME) & KEY_PRESSED) != 0;

                if (ctrlPressed && homePressed)
                {
                    if (!hotKeyPressed)
                    {
                        hotKeyPressed = true;
        ...
        // 显示程序信息方法
        private static void ShowProcessInfo(string targetProcessName)
        {
            ...
            string message = "         版本:  V4.9.0" + "\n" + "         进程:  " + processName + ".exe" + "\n" + "         PID:   " + processId;

            // 使用MessageBox显示信息
            MessageBox(IntPtr.Zero, message, "关于", 0);
        }
    }
