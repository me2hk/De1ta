...
namespace CombatControl 
{
    ...
    public class FireControl 
    {
        ...
        public static bool isBreathEnabled = false;  // 瞄准屏息开关值
            ...
            while(true) 
            {
                ...
                if(currentNumpadAdd && !lastNumpadAddState) 
                {
                    ...
                    if(!isRecoilEnabled) 
                    {
                        ...
                        {
                            if (verticalRecoilPixels > 50)
                            {
                                verticalRecoilPixels = 50;  // 超过50自动修正为50
                                Console.WriteLine("[+] 偏移像素: "+"\x1B[31m错误(大于50px)\x1B[0m");
                            }
                ...
                if(isRecoilEnabled) 
                {
                    ...
                    {
                        int actualPixels = Math.Min(Math.Max(verticalRecoilPixels, 1), 50);// 下移像素值范围限制1-50px
                ...
                if(isStutterStepEnabled) 
                {
                    ...
                    else if((!wPressed || !capsLockPressed) && isStutterActive)
                    {
                        ...
                        keybd_event((byte)VK_SHIFT, 0x2A, KEY_UP_FLAG, 0); //确保Shift释放
...
try {
    ...
    # 计算DPI缩放比例
    $dpi = [Math]::Round([CombatControl.WindowAPI]::GetDeviceCaps($hdc, 88) / 96.0, 2)
    ...
    $windowHeight = [Math]::Round(21 * $charHeight)
...
Write-Host ""
Write-Host ""
Write-Host "[+] 自动压枪 (偏移像素:1-50px)"
Write-Host "[-] 瞄准屏息 (固定映射:Pause/Insert键)"
Write-Host "[*] 自动碎步 (固定映射:Shift键)"
...
try {
    ...
    if($_.Exception.NativeErrorCode -eq 2) {
        Write-Host "[?] 游戏进程 (路径无效)"
        Write-Host "" 
    }
    else {
        Write-Host "[?] 游戏进程 (权限不足)"
        Write-Host "" 
    }
}
catch {
        Write-Host "[?] 游戏进程 (未知错误)"
        Write-Host "" 
}
