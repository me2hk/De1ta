                ...
                if (triggerActive && currentWeapon != null && isRecoilEnabled)
                {
                    ...
                    if (currentStage <= currentWeapon.Stages.Length)
                    {
                        ...
                        if (currentStage <= currentWeapon.Stages.Length)
                        {
                            ...
                            // 8%概率触发突发延迟
                            if (rand.Value.NextDouble() < 0.08)             // 8%概率
                            {
                                delay = rand.Value.Next(50, 91);            // 50-90ms随机延迟
                            }
                            else
                            {
                                delay = (int)(gaussian * 8 + 35);           // 高斯分布参数：μ=35ms(均值), σ=8ms(标准差)
                                ...
                                // 随机延迟范围限制
                                delay = Math.Max(15, Math.Min(delay, 55));  // 允许临时超出5ms
...
# 定义API请求地址
$apiUrl = "https://df.sanyueqi.cn/api/v1/system_data/public?type=map_pwd"

try {
    # 发送GET请求获取API数据（强制错误时停止执行）
    $response = Invoke-RestMethod -Uri $apiUrl -Method Get -ErrorAction Stop

    # 生成带ANSI颜色格式的条目数组
    $entries = $response.data.system_map_pwd | ForEach-Object {
        # 使用ANSI转义码设置绿色文本（格式：\e[32m内容\e[0m）
        "$([char]27)[32m$($_.name):$($_.pwd)$([char]27)[0m"  # 名称和密码用空格分隔
    }

    # 生成绿色格式的括号和占位符
    $leftBracket = "$([char]27)[32m开启 ($([char]27)[0m"  # 左括号绿色
    $rightBracket = "$([char]27)[32m)$([char]27)[0m"      # 右括号绿色
    $separator = "$([char]27)[32m  $([char]27)[0m"        # 占位符绿色

    # 拼接所有元素
    $passwordLine = $leftBracket + ($entries -join $separator) + $rightBracket
    
    # 分步输出内容
    Write-Host "[?] 每日密码: " -NoNewline  # 标签保持默认颜色
    Write-Host $passwordLine  # 输出拼接内容
