...
namespace WindowControl 
{
    ...
    public class InputControl
    {
        ...
        private static bool lastQOrEState = false;    // 跟踪瞄准状态
        ...
        public static void Start() 
        {
            while(true) 
            {
                ...
                // 单个按键状态声明
                bool leftButtonState = (GetAsyncKeyState(VK_LBUTTON) & KEY_PRESSED_FLAG) != 0;   // 鼠标左键
                bool qState = (GetAsyncKeyState(VK_Q) & KEY_PRESSED_FLAG) != 0;                  // Q键
                bool wPressed = (GetAsyncKeyState(VK_W) & KEY_PRESSED_FLAG) != 0;                // W键
                bool eState = (GetAsyncKeyState(VK_E) & KEY_PRESSED_FLAG) != 0;                  // E键
                bool capsLockPressed = (GetAsyncKeyState(VK_CAPITAL) & KEY_PRESSED_FLAG) != 0;   // Caps Lock键

                // 组合按键状态声明
                bool qOrEState = qState || eState;                                               // Q/E键
                bool currentBreathState = qOrEState && leftButtonState;                          // Q/E+鼠标左键
                ...
                {
                    Thread.Sleep(rand.Value.Next(1, 26));               // 按键随机1-25ms
                ...
                else if((!wPressed || !capsLockPressed) && isStutterActive)
                {
                    ...
                    Thread.Sleep(rand.Value.Next(1, 26));               // 随机1-25ms
                ...
                if(isRecoilEnabled) 
                {
                    ...
                    if(qOrEPressed && mouseState) 
                    {
                        ...
                        int horizontalOffset = rand.Value.Next(-1, 2); // 水平像素值随机整数(整体向左偏移)
                        ...
                        //添加噪声
                        int noise = rand.Value.Next(-2, 3); // ±3ms随机干扰
                        delay += noise;   // 应用噪声
