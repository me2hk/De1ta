...
namespace CombatControl 
{
    ...
    public class FireControl 
    {
        ...
        public static bool isPrecisionMode = true;         // 弹道精修开关值

        // 默认压枪参数
        private const int DEFAULT_PIXELS = 11;             // 一段垂直偏移默认值
        public static int precisionDelay = 400;            // 延迟默认值
        public static int precisionOffset = 8;             // 二段垂直偏移默认值
        ...
        public static void Start() 
        {
            while(true) 
            {
                ...
                if (currentDivide && !lastDivideState)
                {
                    ...
                    if ((GetAsyncKeyState(VK_DIVIDE) & KEY_PRESSED_FLAG) != 0)
                    {
                        if (!isPrecisionMode) 
                        {
                            ...
                            Console.WriteLine("[/] 弹道精修: \x1B[32m开启 ("+precisionDelay+"ms/"+precisionOffset+"px)\x1B[0m");
                        }
                        else 
                        {
                            // ===== 第二次按下：直接关闭 =====
                ...
                lastDivideState = currentDivide;  // 重置状态跟踪
                ...
                // 弹道精修模式
                if(isPrecisionMode && elapsedMs >= adjustedDelay)  // 使用随机干扰后的延迟
                {
                    // 生成随机水平偏移（+1px）
