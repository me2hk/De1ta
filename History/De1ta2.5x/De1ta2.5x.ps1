            ...
            while(true) 
            {
                ...
                if (triggerActive && currentWeapon != null && isRecoilEnabled)
                {
                    if (currentStage == 0)
                    {
                        ...
                        .Select(s => s.Duration + rand.Value.Next(-50, 201))
                        .ToArray();
                    }
                    ...
                    if (currentStage <= currentWeapon.Stages.Length)
                    {
                        ...
                        if (currentStage <= currentWeapon.Stages.Length)
                        {
                            ...
                            {
                                delay = rand.Value.Next(60, 101);           // 60-101ms随机延迟
                            }
                            else
                            {
                                ...
                                // 随机延迟范围限制
                                delay = Math.Max(25, Math.Min(delay, 65));  // 限制到目标范围
...
try 
{
    ...
    $windowHeight = [Math]::Round(24 * $charHeight)  # 22行包含缓冲区
